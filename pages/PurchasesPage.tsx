...
import { PlusIcon, ChevronDownIcon, ReportsIcon } from '../components/IconComponents';
...
const PurchasesPage: React.FC = () => {
    ...
    const columns = [
        ...
        { 
            header: 'Vendor Name', 
            accessor: 'vendor' as keyof PurchaseOrder, 
            sortable: true, 
            render: (item: PurchaseOrder) => item.vendor.name,
            sortFn: (a: PurchaseOrder, b: PurchaseOrder) => a.vendor.name.localeCompare(b.vendor.name) 
        },
        ...
        { header: 'Total Amount', accessor: 'total' as keyof PurchaseOrder, sortable: true, render: (item: PurchaseOrder) => `${item.currency} ${item.total.toLocaleString(undefined, {minimumFractionDigits: 2, maximumFractionDigits: 2})}` },
    ...
    const exportColumns = columns.map(c => ({ header: c.header, accessor: c.accessor as keyof(typeof exportableData[0]) }));
        ...
        <div className="space-y-6">
            <div className="flex justify-between items-center">
                ...
                <div className="flex items-center space-x-2">
                    ...
                    <button onClick={() => navigate('/purchase-record')} className="flex items-center px-4 py-2 bg-gray-600 text-white rounded-lg hover:bg-gray-700 no-print">
